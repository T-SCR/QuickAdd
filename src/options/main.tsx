      ...
      <section style={{ marginBottom: '32px' }}>
        ...
        <div style={{ display: 'grid', gap: '10px' }}>
          <div style={{ fontSize: '14px', color: '#475569' }}>
            ...
            {auth?.tokenExpiry ? ` • token expires ${new Date(auth.tokenExpiry).toLocaleString()}` : ''}
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <button type="button" disabled={authBusy} onClick={handleConnectGoogle} style={{ padding: '8px 16px' }}>
              {authBusy ? 'Working…' : (auth?.connected ? 'Reconnect Google' : 'Connect Google')}
            </button>
          ...
          <div style={{ fontSize: '12px', color: '#64748b' }}>
            Tip: Add the Redirect URI above to your OAuth client’s Authorized redirect URIs and add the extension ID to Authorized Chrome apps.
          </div>
        </div>
      </section>
